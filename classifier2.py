...
class CLASSIFIER:
    ...
    def fit(self):
        ...
        for epoch in range(self.nepoch):
            ...
            if acc_unseen>0 or acc_seen>0:
                H = 2*acc_seen*acc_unseen / (acc_seen+acc_unseen)

            #print('acc_seen=%f, acc_unseen=%f, h=%f' % (acc_seen, acc_unseen, H))
            if H > best_H:
                best_seen = acc_seen
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            idx = (test_label == i)
            acc_per_class += (torch.sum(test_label[idx]==predicted_label[idx]) / torch.sum(idx).float())
